...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        const response = await fetch(`${config.API_URL}/api/report/salesChart`, {
            method: 'GET',
            headers: {
                'Authorization': `${token}`,
            }
                ...
                options: {
                    ...
                    scales: {
                        y: {
                            beginAtZero: true,
                            ticks: {
                                stepSize: 1, // This forces the step size to be 1 unit
                                precision: 0, // This ensures only integers are displayed
